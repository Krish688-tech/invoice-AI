...
const StatusBadge = ({ status ="", size = "default", showIcon = true }) => {
  ...
  return (
    <div className={`inline-flex items-center ${sizeClasses[size]} rounded-full font-medium ${config.bg} ${config.text}
    border ${config.border} transition-all duration-300 ease-out hover:scale-150 hover:shadow-sm group relative overflow-hidden`}>
...
export function StatusWithCount({status, count, size="default"}) {
    return (
        <div className="inline-flex items-center gap-2 group">
            ...
            {count !== undefined && (
                <span className="text-xs text-gray-500 font-medium bg-gray-100 px-2 py-1 rounded-full group-hover:bg-gray-200 
